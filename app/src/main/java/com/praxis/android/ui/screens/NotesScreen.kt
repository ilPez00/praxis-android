...
@Composable
fun NotesScreen(entries: List<NotebookEntry>, loading: Boolean, onEntryClick: (String) -> Unit) {
    if (loading) {
    ...
    LazyColumn(modifier = Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        if (entries.isEmpty()) {
            item {
                Text(text = "No notes yet.", modifier = Modifier.padding(16.dp))
            }
        }
        items(entries) { entry ->
            PraxisCard(modifier = Modifier.fillMaxWidth(), onClick = { onEntryClick(entry.id) }) {
